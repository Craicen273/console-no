...
Hooks.on('renderConsoleApp', (app, html, data) => {
    // 1. Safety check
    if (!canvas.ready || !html.closest('#hud').length) return;

    // 2. Hide immediately to prevent the scroll-jump flicker
    html.css({ visibility: "hidden" });

    // 3. Get Map, App Dimensions, & Offsets
    const { x, y, width: sceneWidth } = canvas.dimensions.sceneRect;
	
	// PULL DIRECTLY FROM SETTINGS (Match the ID and Key from your _updateObject)
    const config = game.settings.get(Console.ID, 'defaultConfig');
    const styling = config?.styling || {};

    const appWidth = styling.width || 300;
    const appHeight = styling.height || "auto";
    
    // Ensure these are treated as numbers for the math below
    const offsetX = Number(styling.xOffset) || 0;
    const offsetY = Number(styling.yOffset) || 0;

    // 4. Apply Position (with offsets) & Pointer Events
    html.css({
        position: "absolute",
        left: `${x + offsetX}px`,
        top: `${y + offsetY}px`,
    ...
    // 5. Immediate Scroll to Bottom
    const scrollContainer = html.find('.console-message-container')[0];
    if (scrollContainer) {
    ...
    setTimeout(() => {
        ...
        const input = html.find('input, textarea').first();
        input.focus();
        ...
        if (data.users.includes(game.userId)) {
            switch (data.event) {
                ...
                case 'shareApp':
					...
					// NEW: Check for the flag and nuke the old HTML if it exists
					if (data.forceClear) {
						const oldElement = document.getElementById(`console-container-${data.id}`);
