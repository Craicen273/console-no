...
export default class ConsoleManager extends FormApplication {
    ...
    _handleButtonClick = async (event) => {
        ...
        const consoleData = ConsoleData.getConsole(id)
        ...
        switch (action) {
            case 'create':
                ...
                break;
            case 'open-console':
                const appWindow = document.getElementById(consoleData.id)
                if (!appWindow) {
					
					// 1. Identify the specific HTML ID used for this console
					// Check your ConsoleApp defaultOptions for the exact prefix
					const htmlId = `console-container-${consoleData.id}`; 
					
					// 2. Remove it from the DOM directly
					const existingElement = document.getElementById(htmlId);
					if (existingElement) {
						existingElement.remove();
					}
					
                    // 1. Render app for the GM locally
					const myApp = new ConsoleApp(consoleData.id);
					myApp.render(true);

					// 2. AUTOMATIC SHARE: Broadcast to all players if the GM opens it
					if (game.user.isGM) {
						game.socket.emit('module.console', {
							event: "shareApp",
							forceClear: true,
							id: consoleData.id, // Ensure this matches the ID expected by the socket listener
							users: game.users.map(u => u.id)
						});
					}
                } else {
                ...
                if (flags.includes(consoleData.id)) {
                    await ConsoleData.removeFromPlayerFlags('messageNotification', [game.userId], consoleData.id)
