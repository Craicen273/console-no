...
export default class ConsoleApp extends FormApplication {
	...
	_injectHTML(html) {
		const container = $("#hud");
		if (container.length) {
			container.find(`.console-app-${this.consoleId}`).remove();
	...
	async _onSubmit(event, {updateData=null, preventClose=false, preventRender=false}={}) {
		...
		// 2. Run the standard Foundry submission logic
		// This triggers your _updateObject and validation
		const formData = await super._onSubmit(event, {updateData, preventClose, preventRender});

		// 3. Force a re-render to the HUD
		this.render();
	...
	getData() {
		...
		// OVERWRITE the template data with your saved settings
		if (config?.styling) {
			// This ensures data.styling in the .hbs is your actual config
			data.styling = foundry.utils.mergeObject(data.styling || {}, config.styling);
		}
		...
		// Force these into position so _injectHTML can see them immediately
		this.position.width = styling.width;
		this.position.height = styling.height;

		// Apply to options as well
		this.options.width = styling.width;
		this.options.height = styling.height;
